...
-- Check if notification_emails column exists and what type it is
DO $$
BEGIN
    -- First, check if the column exists
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'settings' AND column_name = 'notification_emails'
    ) THEN
        -- Add as JSONB if it doesn't exist
        ALTER TABLE settings ADD COLUMN notification_emails JSONB DEFAULT '[]'::jsonb;
    ELSE
        -- If it exists but is text[], convert it to JSONB
        IF EXISTS (
            SELECT 1 FROM information_schema.columns 
...
-- Update notification_emails only if it's null or empty
UPDATE settings 
SET notification_emails = '[]'::jsonb
